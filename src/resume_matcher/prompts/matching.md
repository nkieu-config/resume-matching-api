...
4 means strong professional application with clear ownership, depth, leadership, production scale, or measurable outcome.

Level zero must contain no evidence identifiers (evidence_ids must be empty []). Every positive level must contain at least one existing evidence identifier. Use not evidenced rather than claiming the candidate does not have a capability.
...
Score experience.requirements as zero unless the evidence directly supports gathering, clarifying, or validating requirements with users, stakeholders, clients, or business teams. Score experience.production as zero when production deployment is not evidenced, absent, or explicitly negated (e.g. phrases with ยังไม่ or not yet deploy); engineering collaboration alone without production deployment must be scored as level zero. For tools.automation, n8n is preferred but not required: GitHub Actions, CI/CD, and other explicit automation pipelines are valid evidence.

For positive levels, every statement in a rationale must be directly supported by its selected evidence identifiers. For level zero, state the absence of evidence or negated claim in Thai without citing any evidence identifiers. Do not infer an action from a technology name in a project stack: a listed tool supports project exposure or use, but not an unstated packaging, operation, data-structure, or evaluation claim. For experience.evaluation, distinguish general application tests from evaluation of AI output quality and state the missing AI evaluation evidence as a gap. For experience.production, select evidence that directly supports both AI delivery and engineering collaboration, and state when production deployment is not explicit. For tools.api_json, REST APIs or API frameworks (e.g. FastAPI) qualify as API evidence; do not claim explicit JSON schemas unless supported by evidence. For education.relevant, assess direct field relevance: level 1 for online courses, self-study, or training mentioned, level 2 for relevant coursework or undergraduate degree, level 3 for completed relevant degree (such as MSc Data Science or Computer Science) or professional certification, and level 4 for advanced specialized degree (such as MSc Data Science) combined with relevant certifications. Describe education without inferring whether study is ongoing or completed. Check Thai grammar and spacing before returning the result.
