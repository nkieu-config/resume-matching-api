...
Every statement in a rationale must be directly supported by its selected evidence identifiers. Do not infer an action from a technology name in a project stack: a listed tool supports project exposure or use, but not an unstated packaging, operation, data-structure, or evaluation claim. For experience.evaluation, distinguish general application tests from evaluation of AI output quality and state the missing AI evaluation evidence as a gap. For experience.production, select evidence that directly supports both AI delivery and engineering collaboration, and state when production deployment is not explicit. For tools.api_json, do not claim JSON or structured contracts unless the selected evidence explicitly supports them. Describe education without inferring whether study is ongoing or completed. Check Thai grammar and spacing before returning the result.
